  ...
  methods: {
    ...
    async onFileChange(event) {
      ...
      if (file) {
        ...
        try {
          ...
          const response = await axios.post(`http://localhost:8080/api/User/${this.user.id}/upload-profile-picture`, formData, {
            headers: {
    ...
    async saveProfile() {
      try {
        ...
        await axios.put(`http://localhost:8080/api/User/${this.user.id}`, this.user);
